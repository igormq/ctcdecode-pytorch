...
def data():
    ...
    beam_size = 20
    log_probs_seq1 = torch.as_tensor([[0.1649, 0.0639, 0.2112, 0.2732, 0.0687, 0.0361, 0.1818],
    ...
    log_probs_seq2 = torch.as_tensor([[0.1090, 0.0803, 0.2267, 0.0580, 0.3681, 0.1131, 0.0447],
...
def test_ctc_beam_search_decoder_tf():
    log_input = torch.tensor([[0, 0.6, 0, 0.4, 0, 0], [0, 0.5, 0, 0.5, 0, 0], [0, 0.4, 0, 0.6, 0, 0],
...
def test_beam_search_decoder_batch(data):
    vocab_list, beam_size, log_probs_seq1, log_probs_seq2, _, beam_search_result = data
    beam_results = decoders.ctc_beam_search_decoder([log_probs_seq1, log_probs_seq2],
                                                    beam_size=beam_size,
                                                    num_processes=24)
...
def test_ctc_decoder_beam_search_different_blank_idx():
    ...
    assert np.alltrue(results[1][1] == [1, 2, 1])
    assert np.allclose(4.73437, results[0][0])
    # tf results: 7.0157223, 0.584855
    assert np.allclose(4.93009, results[1][0])
    # tf results: 7.8031697, 0.389139
...
def test_real_ctc_decode():
    ...
    # lm-based decoding
    scorer = KenLM(os.path.join(data_dir, 'bigram.arpa'), tokenizer, unit=LMUnit.Word)
...
def test_real_ctc_decode2():
    ...
    tokenizer = Tokenizer(os.path.join(data_dir, 'vocab.txt'))
    tokenizer.blank_index = tokenizer.get_index('<blank>')
    tokenizer.space_index = tokenizer.get_index('<space>')

    beam_size = 16

    result = decoders.ctc_beam_search_decoder(seq, blank=tokenizer.blank_index, beam_size=beam_size)
    ...
    # lm-based decoding
    scorer = KenLM(os.path.join(data_dir, 'ctc-test-lm.binary'),
                   tokenizer,
                   unit=LMUnit.Word,
                   trie_path=os.path.join(data_dir, 'ctc-test-lm.trie'))
