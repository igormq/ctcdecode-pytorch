...
class KenLM(_C.LM):
    ...
    def start(self, start_with_nothing):
        ...
        print('deu start')
        return _C.to_shared_ptr(KenLMState(state=state, tokens=[]))

    def score(self, state, token_index):
        print('score')
        if token_index != self.tokenizer.space_index:
            state.state += [token_index]
            return KenLMState(state=copy.copy(state.state), tokens=state.tokens), -float('inf')
        ...
        return KenLMState(state=new_state, tokens=[]), score

    def finish(self, state):
        print('finish')
        score = 0.0
        if len(state.tokens):
            state.state, score = self.score(state.state, self.tokenizer.space_index)
