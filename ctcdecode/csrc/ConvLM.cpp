...
std::pair<LMStatePtr, float> ConvLM::scoreWithLmIdx(
    ...
    const int tokenIdx)
{
    auto inState = std::static_pointer_cast<ConvLMState>(state).get();
...
void ConvLM::updateCache(std::vector<LMStatePtr> states)
{
    ...
    {
        auto state_ = std::static_pointer_cast<ConvLMState>(state).get();
...
int ConvLM::compare(const LMStatePtr &state1, const LMStatePtr &state2)
    const
{
